...
async def query_manager(client: Client, query: CallbackQuery):
    ...
    # order_404 its for when the admin can't find the order and the user needs to be notified
        ...
        try:
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "🎬Canal de pedidos🎬",
                                url=f"https://t.me/{os.getenv('ORDERS_ID')}",
                            )
                        ]
                    ]
                ),
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "🫡Orden Lista🫡",
                                callback_data=f"order_ready_{message_replied_id}",
                            ),
                            InlineKeyboardButton(
                                "❌No encontrado❌",
                                callback_data=f"order_404_{message_replied_id}",
                            ),
                        ]
                    ]
                ),
            ...
            await query.answer(
                "Tu orden fue enviada a los administradores ✅", show_alert=False
            )
        except Exception as e:
            logger.error(f"Error en order_not_found -> {e}")
            await query.answer(
                "Ocurrió un error al reenviar tu orden.", show_alert=True
            )
...
"""
        buttons = [
            [
                InlineKeyboardButton(
                    "💳 CUP Metropolitano", callback_data="pay_edit_metro"
                )
            ],
            ...
            await query.message.reply_photo(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "💜QvaPay PayMe Link", url=os.getenv("QVAPAY_LINK")
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                "Confirmar✅", callback_data="confirm_pay"
                            )
                        ],
                    ]
                ),
        ...
        if photo_id:
            ...
            buttons = [
                [
                    InlineKeyboardButton(
                        "Aceptar Pago✅",
                        callback_data=f"premium_accept_{user_info.username or 'Sin username'}_{user_id}",
                    ),
                    InlineKeyboardButton(
                        "Declinar Pago❌",
                        callback_data=f"premium_decline_{user_info.username or 'Sin username'}_{user_id}",
                    ),
                ]
            ]
            ...
            await query.message.edit_text(
                "✅ *¡Comprobante enviado a los administradores!*"
            )
        else:
        ...
        if splitted_data[1] == "accept":
            ...
            await client.send_message(
                ...
                text=(
                    ...
                    "👉 @{group_id}\n\n"
                    "Estamos aquí para ayudarle.".format(group_id=os.getenv("GROUP_ID"))
                ),
            )
