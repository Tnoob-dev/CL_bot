    ...
    if len(message.command) >= 2:
        ...
        if len(results) > 0:
            keyboard = InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton(text=res.get("name"), url=res.get("link"))]
                    for res in results
                ]
                + [
                    [
            ...
            await message.reply(
                "Segun tu peticion encontre esto", reply_markup=keyboard
            )
        else:
            mes = await message.reply(
                "✅Orden Reenviada al canal de pedidos✅",
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "🎬Canal de pedidos🎬", url="https://t.me/CinemaOrders"
                            )
                        ]
                    ]
                ),
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "🫡Orden Lista🫡",
                                callback_data=f"order_ready_{mes.reply_to_message_id}",
                            ),  # reply_to_message_id is deprecated, investigate about reply_parameters
                            InlineKeyboardButton(
                                "❌No encontrado❌",
                                callback_data=f"order_404_{mes.reply_to_message_id}",
                            ),
                        ]
                    ]
                ),
